...
class API:
    ...
    def search(self, 
        ...
        if self.checker.passed:
            ...
            url = f"https://www.ebay.com/sh/research/api/search?marketplace=EBAY-US&keywords={search_keyword}&dayRange={day_range}&endDate={endDate}&startDate={startDate}&categoryId={arg_parser.choose_category(category)}&offset=0&limit=50&sorting={sorting_option}&tabName={post_status}&tz=America%2FNew_York&modules=aggregates&modules=searchResults&modules=resultsHeader"
            self.handle_data_retrieval(url, search, post_status, output_format, output_folder)
            
def should_skip(should_skip):
    if should_skip == "SKIP":
        return True
    return False
            
def search_wrapper(api: API):
...
def main():          
    api = API()              
    key_combinations = {
    (keyboard.Key.ctrl, keyboard.KeyCode(char='s')): search_wrapper(api),
    (keyboard.Key.ctrl, keyboard.KeyCode(char='n')): api.next_page
    }
    def on_press(key):
    # Check if the key combination is in the key_combinations dictionary
        for combo, keys in key_combinations.items():
            if all(k == k2 for k, k2 in zip(combo, (key, *keys))):
